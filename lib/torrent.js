...
let TMP
try {
  TMP = path.join(fse.statSync('/tmp') && '/tmp', 'webtorrent')
} catch (err) {
...
class Torrent extends EventEmitter {
    ...
    if (this.skipVerify) {
      ...
      if (this._fileModtimes && this._store === FSChunkStore) {
        ...
        this.getFileModtimes((err, fileModtimes) => {
          ...
          this._verifyFiles((err, negativeVerified) => {
            if (err) return onPiecesVerified(err)
      
            const unchanged = this.files
              .map((_, index) => fileModtimes[index] === this._fileModtimes[index])
              .every((x) => x)

            if (unchanged && negativeVerified) {
              this._markAllVerified()
              this._onStore()
            } else {
  ...
  getFileModtimes (cb) {
    ...
    parallelLimit(this.files.map((file, index) => cb => {
      fse.stat(path.join(this.path, file.path), (err, stat) => {
  ...
  _checkToNegativeFiles (cb) {
    parallelLimit(
      this.files.map((file) => (cb) => {
        if (this.destroyed) return cb(new Error('torrent is destroyed'))
        const filePath = path.resolve(this.path, file.path)

        fse.stat(filePath, (err, stat) => {
          if (this.destroyed) return cb(new Error('torrent is destroyed'))
          if (err && err.code !== 'ENOENT' && err.code !== 'EPERM' && err.code !== 'UNKNOWN') {
            return cb(err)
          }
          if (stat && !stat.isFile()) return cb(null, null)

          const verified = stat ? stat.size === file.length : false
      ...
      (err, files) => {
        const negativeFiles = files.filter(Boolean)
        return cb(err, { negative: negativeFiles.length, files: negativeFiles.filter(Boolean) });
      }
    )
  }

  _unlinkFiles (files, cb) {
    parallelLimit(
      files.map((file) => (cb) => {
        if (this.destroyed) return cb(new Error('torrent is destroyed'))
        const filePath = path.resolve(this.path, file.path)
  ...
  _verifyFiles (cb) {
    this._checkToNegativeFiles((err, result) => {
      if (err) return cb(err)
      if (result.negative <= 0) return cb(null, true)

      this._unlinkFiles(result.files, (err) => {
        if (err) return cb(err)
  ...
  _updateWireWrapper (wire) {
    ...
    if (typeof window !== 'undefined' && typeof window.requestIdleCallback === 'function') {
      self.wireIdleCallbacks = self.wireIdleCallbacks || []
      if (!self.wireIdleCallbacks.includes(wire)) {
        self.wireIdleCallbacks.push(wire)
  ...
  _updateWire (wire) {
    ...
    self.wireIdleCallbacks = self.wireIdleCallbacks || []
    self.wireIdleCallbacks = self.wireIdleCallbacks.filter(obj => obj !== wire)

    if (wire.peerChoking) return
    if (!wire.downloaded) return validateWire()

    const minOutstandingRequests = Math.min(
      isWebSeed
        ? Math.min(
          getPiecePipelineLength(wire, PIPELINE_MIN_DURATION, self.pieceLength),
          self.maxWebConns
        )
        : getBlockPipelineLength(wire, PIPELINE_MIN_DURATION),
      self.client.maxConns
    )

    if (wire.requests.length >= minOutstandingRequests) return

    const maxOutstandingRequests = Math.min(
      isWebSeed
        ? Math.min(
